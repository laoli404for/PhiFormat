...
在 PhiFormat 中，tick 为整数值，从 0 开始计算。0 tick 被视为曲目的起始（0 second）。每一拍根据 **时基 (Timing Base)** 被划分为指定数量的 tick 数。默认情况下，时基为 48。48 分能够满足绝大多数的谱面制作需求。为保证兼容性，我们建议谱师不要修改默认的时基。

!!! warning
    编辑器作者请注意，您可以在编辑器中提供修改时基的选项，但应明确指出不建议修改。
